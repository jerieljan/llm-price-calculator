...
def filter_models(df, includes, excludes):
    ...
    filtered = df[df.apply(
        lambda row: (
            (not includes or any(include.lower() in row['id'].lower() or include.lower() in row['name'].lower() for include in includes)) and
            (not excludes or all(exclude.lower() not in row['id'].lower() and exclude.lower() not in row['name'].lower() for exclude in excludes))
        ),
        axis=1
    )]
    return filtered
